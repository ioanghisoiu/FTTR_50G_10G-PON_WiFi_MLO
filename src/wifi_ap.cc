...
class WiFi_AP : public cSimpleModule
{
    private:
        ...
        simsignal_t latencySignalXr_wifi;
        simsignal_t latencySignalHmd_wifi;
        simsignal_t latencySignalCtr_wifi;
        simsignal_t latencySignalHpt_wifi;
        simsignal_t latencySignalBkg_wifi;
...
void WiFi_AP::initialize()
{
    ...
    latencySignalXr_wifi = registerSignal("xr_latency_wifi");
    latencySignalHmd_wifi = registerSignal("hmd_latency_wifi");
    latencySignalCtr_wifi = registerSignal("ctrl_latency_wifi");
    latencySignalHpt_wifi = registerSignal("hptc_latency_wifi");
    latencySignalBkg_wifi = registerSignal("bkg_latency_wifi");
...
void WiFi_AP::handleMessage(cMessage *msg)
{
    if(msg->isPacket() == true) {
        if(strcmp(msg->getName(),"bkg_data") == 0) {                // updating buffer size after receiving requests from ONUs
            ...
            emit(latencySignalBkg_wifi, bkg_packet_latency);
            ...
            emit(latencySignalXr_wifi, xr_packet_latency);
            ...
            emit(latencySignalHmd_wifi, hmd_packet_latency);
            ...
            emit(latencySignalCtr_wifi, ctrl_packet_latency);
            ...
            emit(latencySignalHpt_wifi, hptc_packet_latency);
